...
export class EnemyModel extends ObjectModel {
    ...
    distanceUpdate() {
        ...
        // this.#distance = Math.floor(Math.hypot(row - Player.row, col - Player.col));
    ...
    #hunt() { // !!! рабочий метод
         ...
         * @typedef {Object} Step
         * @property {CellModel} cell текущая клетка
         * @property {number} distance расстояние между текущей клеткой и игроком
         * @property {CellModel[]} way путь между стартовой и текущей клетками 
        */

        /** Создает Шаг от текущей клетки к игроку 
         * @param {CellModel} cell текущая клетка 
         * @param {CellModel[]} way путь между стартовой и текущей клетками 
         * @returns {Step}
         */
         ...
         * @param {Step[]} array - массив с Шагами
         * @param {boolean} mode - нужно ли оптимизировать путь от начальной клетки к текущему шагу
         * @returns {boolean}
         */
         ...
         * @type {Array<Step>} 
         */
        // const stepsQueue = this.#cellsAround().map(cell => stepMaker(cell))
